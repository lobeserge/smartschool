	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		try {
			...
			if (jwt != null && jwtUtils.validateJwtToken(jwt)) {
				...
				UserDetails userDetailsAdmin = adminDetailsService.loadUserByUsername(admin);
				UsernamePasswordAuthenticationToken authentication_admin = new UsernamePasswordAuthenticationToken(
						userDetailsAdmin, null, userDetailsAdmin.getAuthorities());

				authentication_admin.setDetails(new WebAuthenticationDetailsSource().buildDetails(request));
